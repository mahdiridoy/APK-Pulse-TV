...
import com.pulsestream.app.ui.APIRepository.Companion.noneApi
import com.pulsestream.app.ui.APIRepository.Companion.randomApi
import com.pulsestream.app.ui.WatchType
import com.pulsestream.app.ui.quicksearch.QuickSearchFragment
import com.pulsestream.app.ui.search.SEARCH_ACTION_FOCUSED
...
import com.pulsestream.app.utils.AppContextUtils.addProgramsToContinueWatching
import com.pulsestream.app.utils.AppContextUtils.filterHomePageListByFilmQuality
import com.pulsestream.app.utils.AppContextUtils.filterProviderByPreferredMedia
import com.pulsestream.app.utils.AppContextUtils.filterSearchResultByFilmQuality
...
import com.pulsestream.app.utils.DOWNLOAD_HEADER_CACHE_BACKUP
import com.pulsestream.app.utils.DataStoreHelper
import com.pulsestream.app.utils.DataStoreHelper.deleteAllResumeStateIds
import com.pulsestream.app.utils.DataStoreHelper.getAllResumeStateIds
...
import com.pulsestream.app.utils.DataStoreHelper.getCurrentAccount
...
class HomeViewModel : ViewModel() {
    ...
    private val expandable: MutableMap<String, ExpandableHomepageList> = ConcurrentHashMap()
    private val _page =
    ...
    val lock: MutableSet<String> = Collections.synchronizedSet(mutableSetOf())
    ...
    private fun load(api: MainAPI): Job = ioSafe {
        ...
        val cachedResponse = HomePageCache.get(api.name)
        if (cachedResponse != null) {
            applyHomepageData(cachedResponse, api)
            com.pulsestream.app.diagnostics.PulseDiagnostics.event(
                "CACHE", "provider=${api.name}", "source=memory_or_disk", "sections=${cachedResponse.items.size}"
            )
        } else {
        ...
        // ---- Background refresh: fetch fresh data from network ----
        when (val data = repo?.getMainPage(1, null)) {
            is Resource.Success -> {
                try {
                    // Update cache with fresh data
                    val freshResponse = com.lagradost.cloudstream3.newHomePageResponse(
                        data.value.mapNotNull { it?.items }.flatten(),
                        data.value.any { it?.hasNext == true }
                    )
                    HomePageCache.put(api.name, freshResponse)

                    // Apply fresh data to UI
                    applyHomepageData(freshResponse, api)

                    com.pulsestream.app.diagnostics.PulseDiagnostics.event(
                        "CACHE", "provider=${api.name}", "source=network_fresh",
                        "sections=${freshResponse.items.size}"
                    )
                } catch (e: Exception) {
            ...
            is Resource.Failure -> {
                // Network failed — if we already showed cache, keep it
                if (cachedResponse == null) {
                ...
                // If cache was shown, silently keep it (don't overwrite with error)
            }
    ...
    /**
     * Apply homepage data to the expandable map and preview.
     * Extracted so it can be called from both cache-first and network-fresh paths.
     */
    private suspend fun applyHomepageData(
        ...
        try {
            ...
            response.items.forEach { list ->
                ...
                filteredList.list = AdultContentFilter.filterList(filteredList.list)
            ...
            if (items.isNotEmpty()) {
                ...
                if (currentList.isNotEmpty()) {
                    val randomItems = AdultContentFilter.filterList(
        ...
        ioSafe {
            ...
            if (preferredApiName == noneApi.name) {
                ...
                if (PluginManager.loadedOnlinePlugins || PluginManager.isSafeMode()) {
                    // Default to MovieBox if no homepage is set, otherwise fall back to None
                    val movieBoxApi = getApiFromNameNull("MovieBox")
                    if (movieBoxApi != null && preferredApiName == null) {
                        DataStoreHelper.currentHomePage = movieBoxApi.name
                        loadAndCancel(movieBoxApi)
                    } else {
                        loadAndCancel(noneApi)
                    }
                } else {
