...
import com.pulsestream.app.MainActivity.Companion.deleteFileOnExit
...
import com.pulsestream.app.ui.player.CustomDecoder.Companion.fixSubtitleAlignment
import com.pulsestream.app.ui.player.live.LiveHelper
import com.pulsestream.app.ui.player.live.PREFERRED_LIVE_OFFSET
...
import com.pulsestream.app.ui.subtitles.SaveCaptionStyle
import com.pulsestream.app.ui.subtitles.SubtitlesFragment.Companion.applyStyle
            ...
            exoPlayer?.addListener(object : Player.Listener {
                ...
                override fun onPlayerStateChanged(playWhenReady: Boolean, playbackState: Int) {
                    exoPlayer?.let { exo ->
                        ...
                        // Update the cached flag BEFORE the event is dispatched: the StatusEvent
                        // handler below runs synchronously on the main thread and can call back
                        // into getIsPlaying(), which reads this cached value. If we wrote it
                        // afterwards, a pause transition would still report the stale "playing"
                        // state and a host would never propagate the pause to viewers.
                        isPlaying = exo.isPlaying
                        event(
                            StatusEvent(
                                wasPlaying = if (wasPlaying) CSPlayerLoading.IsPlaying else CSPlayerLoading.IsPaused,
                                isPlaying =
                                    when (playbackState) {
