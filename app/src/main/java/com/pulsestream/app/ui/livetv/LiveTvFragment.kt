...
import com.pulsestream.app.databinding.FragmentLiveTvBinding
...
class LiveTvFragment : BaseFragment<FragmentLiveTvBinding>(
    ...
    private fun setupServerButtons(binding: FragmentLiveTvBinding) {
        val server1Button = binding.liveTvServer1Button
        ...
        server1Button.setOnClickListener {
            viewModel.switchServer(LiveTvRepository.Server.SERVER_1)
        }
        server2Button.setOnClickListener {
            viewModel.switchServer(LiveTvRepository.Server.SERVER_2)
        }

        // Set initial state
        observe(viewModel.selectedServer) { server ->
            server.let { updateButtonStates(it) }
        }
    }
    ...
    /**
     * Moves focus to the server buttons area for TV remote navigation.
     * Returns true if focus was moved.
     */
    private fun focusServerButtons(): Boolean {
        val button = binding?.liveTvServer1Button ?: return false
        button.requestFocus()
        return true
    }
            ...
            if (event == null || event.action != KeyEvent.ACTION_DOWN) {
                ...
                KeyEvent.KEYCODE_DPAD_UP -> {
                    ...
                    if (target >= 0) {
                        ...
                        // Up from the first row always lands on the server buttons
                        focusServerButtons()
                    }
                }
