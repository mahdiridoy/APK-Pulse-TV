package com.pulsestream.app.ui.watchtogether
...
object WatchTogetherManager {
    ...
    /**
     * M4 FIX: Atomic disconnect guard to prevent double-fire of onDisconnected.
     * Uses compareAndSet so only the first thread to flip this wins.
     */
    private val disconnectGuard = java.util.concurrent.atomic.AtomicBoolean(false)
    ...
    fun createRoom(roomName: String, callback: (Result<RoomInfo>) -> Unit) {
        ...
        val request = Request.Builder()
            ...
            .post(body.toRequestBody("application/json".toMediaTypeCompat()))
            .build()
    ...
    /**
     * H1 FIX: Synchronized to prevent race between disconnect() and OkHttp callbacks.
     * M4 FIX: Uses disconnectGuard to prevent double-fire of onDisconnected.
     */
    @Synchronized
    fun disconnect(clearSavedConnection: Boolean = true) {
        // M4 FIX: Only allow one thread to execute disconnect logic
        if (!disconnectGuard.compareAndSet(false, true)) return
        try {
    ...
    /** H1 FIX: Synchronized to prevent send after socket is nulled by concurrent disconnect. */
    @Synchronized
