...
import com.pulsestream.app.MainActivity.Companion.afterRepositoryLoadedEvent
import com.pulsestream.app.R
import com.pulsestream.app.databinding.AddRepoInputBinding
import com.pulsestream.app.databinding.FragmentExtensionsBinding
...
import com.pulsestream.app.ui.settings.SettingsFragment.Companion.setSystemBarsPadding
...
import com.pulsestream.app.utils.AppContextUtils.addRepositoryDialog
            ...
            binding.applyBtt.setOnClickListener secondListener@{
                ...
                ioSafe {
                    try {
                        ...
                        // FIX: Auto-download all plugins from the newly added repository so
                        // the user doesn't have to click each plugin manually. Runs in a
                        // background coroutine; safe to call here because downloadAll uses
                        // ioSafe internally. The downloadAll function already handles the
                        // race condition with the filesystem (delay 200ms before loadPlugin).
                        this@ExtensionsFragment.activity?.let { act ->
                            PluginsViewModel.downloadAll(act, newRepo, pluginViewModel)
                        }
                    } finally {
