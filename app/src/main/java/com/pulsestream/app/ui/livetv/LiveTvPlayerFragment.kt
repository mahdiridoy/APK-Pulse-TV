...
import com.pulsestream.app.databinding.FragmentLiveTvPlayerBinding
...
class LiveTvPlayerFragment : BaseFragment<FragmentLiveTvPlayerBinding>(
    ...
    private fun setupGestures(binding: FragmentLiveTvPlayerBinding) {
        val helper = LiveTvGestureHelper(
            ...
            onTap = { toggleTopBar(binding) },
        )
        binding.liveTvGestureOverlay.setOnTouchListener(helper)
    }

    /** Toggle the top bar (back button, channel name, resolution, etc.) on tap. */
    private var isTopBarVisible = false
    private fun toggleTopBar(binding: FragmentLiveTvPlayerBinding) {
        hideTopBarRunnable?.let { binding.root.removeCallbacks(it) }
        if (isTopBarVisible) {
            binding.liveTvTopBar.animate().alpha(0f).setDuration(200)
                .withEndAction { binding.liveTvTopBar.visibility = View.GONE }.start()
            ...
            val runnable = Runnable {
                binding.liveTvTopBar.animate().alpha(0f).setDuration(300)
                    .withEndAction { binding.liveTvTopBar.visibility = View.GONE }.start()
                isTopBarVisible = false
            }
