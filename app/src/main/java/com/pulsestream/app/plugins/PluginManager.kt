...
import com.pulsestream.app.MainActivity.Companion.afterPluginsLoadedEvent
...
import com.pulsestream.app.plugins.RepositoryManager.ONLINE_PLUGINS_FOLDER
import com.pulsestream.app.plugins.RepositoryManager.PREBUILT_REPOSITORIES
import com.pulsestream.app.plugins.RepositoryManager.downloadPluginToFile
import com.pulsestream.app.plugins.RepositoryManager.getRepoPlugins
import com.pulsestream.app.plugins.RepositoryManager.sha256
import com.pulsestream.app.ui.settings.extensions.REPOSITORIES_KEY
import com.pulsestream.app.ui.settings.extensions.RepositoryData
import com.pulsestream.app.utils.AppContextUtils.getApiProviderLangSettings
...
object PluginManager {
    ...
    /** L2 FIX: Use concurrent set to track all currently-loading plugins (not just the last one). */
    @Volatile
    var currentlyLoading: String? = null
        private set

    /** L2 FIX: Set of all currently loading plugin names for accurate crash reporting. */
    private val loadingPlugins = java.util.concurrent.ConcurrentHashMap.newKeySet<String>()

    /**
     * H2 FIX: Use ConcurrentHashMap for thread-safe concurrent access from
     * plugin loading threads and UI thread reads.
     */
    val plugins: MutableMap<String, BasePlugin> =
        java.util.concurrent.ConcurrentHashMap<String, BasePlugin>()

    /** H2 FIX: ConcurrentHashMap for thread-safe URL-to-plugin mapping. */
...
@Suppress("FunctionName")
    ...
    suspend fun ___DO_NOT_CALL_FROM_A_PLUGIN_updateAllOnlinePluginsAndLoadThem(activity: Activity) {
        ...
        val urls = (getKey<Array<RepositoryData>>(REPOSITORIES_KEY, null)
    ...
    private suspend fun loadPlugin(context: Context, file: File, data: PluginData): Boolean {
        ...
        // L2 FIX: Track in both single field and set for backwards compatibility
        currentlyLoading = fileName
        loadingPlugins.add(fileName)
        ...
        return try {
            ...
            val loader = SafePathClassLoader(filePath, context.classLoader)

            // PRELOAD SHIM CLASSES INTO PARENT CLASSLOADER BEFORE LOADING PLUGIN
            // This forces JVM to resolve host shim versions first, so when the plugin's
            // DEX is opened and its bundled CloudStream3 classes are referenced during
            // verification, the parent classloader already has the correct versions cached.
            // Without this, the JVM resolves the plugin's bundled (incompatible) copies.
            try {
                // Core shim classes that external plugins reference
                val shimClasses = arrayOf(
                    ...
                    "com.lagradost.cloudstream3.Score",
                    "com.lagradost.cloudstream3.syncproviders.SyncRepo",
                    "com.lagradost.cloudstream3.syncproviders.SubtitleRepo",
                    "com.lagradost.cloudstream3.network.CloudflareKiller",
                    ...
                    "com.lagradost.cloudstream3.mvvm.debugPrint",
                    "com.lagradost.cloudstream3.mvvm.logError",
                    "com.lagradost.cloudstream3.mvvm.safe"
                )
                for (className in shimClasses) {
                    try {
                        // Force load via parent classloader (context.classLoader)
                        // SafePathClassLoader.preloadShimClass delegates to parent for these
                        loader.preloadShimClass(className)
                    } catch (e: Throwable) {
                        Log.w(TAG, "Failed to preload shim $className: ${e.message}")
                    }
                }
            } catch (e: Throwable) {
                Log.w(TAG, "Shim preload failed (non-fatal): ${e.message}")
            }
            ...
            // H2 FIX: ConcurrentHashMap is thread-safe, no synchronized needed
            plugins[filePath] = pluginInstance
            classLoaders[loader] = pluginInstance
            urlPlugins[data.url ?: filePath] = pluginInstance
            // Plugin class can be either the app's Plugin (com.pulsestream.app.plugins.Plugin)
            // or the shim Plugin (com.lagradost.cloudstream3.plugins.Plugin) used by external .cs3 plugins.
            // The shim Plugin is the one that has a load(context) override that registers providers.
            if (pluginInstance is com.lagradost.cloudstream3.plugins.Plugin) {
                pluginInstance.load(context)
            } else if (pluginInstance is Plugin) {
                pluginInstance.load(context)
            } else {
                pluginInstance.load()
            }
            Log.i(TAG, "Loaded plugin ${data.internalName} successfully")
            com.pulsestream.app.diagnostics.PulseDiagnostics.pluginEvent(data.internalName, "LOAD_SUCCESS")
            loadingPlugins.remove(fileName)
            currentlyLoading = loadingPlugins.firstOrNull()
            true
        } catch (e: Throwable) {
            val stackTrace = Log.getStackTraceString(e)
            Log.e(TAG, "Failed to load $file: $stackTrace")
            // Internal diagnostic: log the full error for engineers
            // User-facing: show a clean, non-technical message
            val shortReason = when (e) {
                is ClassNotFoundException -> "Class not found: ${e.message?.take(120)}"
                is NoClassDefFoundError -> "Missing class: ${e.message?.take(120)}"
                is VerifyError -> "Verify error (incompatible plugin)"
                is UnsatisfiedLinkError -> "Native lib error"
                is java.lang.reflect.InvocationTargetException ->
                    "Plugin constructor failed: ${e.cause?.javaClass?.simpleName}: ${e.cause?.message?.take(120)}"
                else -> e.javaClass.simpleName + ": " + (e.message?.take(120) ?: "unknown")
            }
            Log.w(TAG, "Plugin load failed [$fileName]: $shortReason")
            com.pulsestream.app.diagnostics.PulseDiagnostics.pluginEvent(fileName, "LOAD_FAILED", "errorType=${e.javaClass.simpleName}", "reason=$shortReason")
            // Do NOT show raw technical errors to the user
            // The full error is in Logcat for engineers
            loadingPlugins.remove(fileName)
            currentlyLoading = loadingPlugins.firstOrNull()
            false
        }
    ...
    suspend fun downloadPlugin(
        ...
        try {
            ...
            return if (loadPlugin) {
                ...
                // FIX: the dex file was just written by Files.move. The filesystem
                // may not have flushed the bytes yet, so a PathClassLoader that tries
                // to read manifest.json immediately can fail with VerifyError or
                // return null, causing the plugin to only work on the next app
                // launch. A short delay lets the page cache settle.
