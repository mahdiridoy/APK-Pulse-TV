...
import com.pulsestream.app.plugins.PluginManager.getPluginSanitizedFileName
import com.pulsestream.app.plugins.PluginManager.unloadPlugin
...
object RepositoryManager {
    ...
    suspend fun parseRepository(url: String): Repository? {
        return safeAsync {
            try {
                ...
                val fixedUrl = convertRawGitUrl(url)
                logError(Exception("Parsing repository: $fixedUrl"))
                val response = app.get(fixedUrl, cacheTime = 5, cacheUnit = TimeUnit.MINUTES)
                logError(Exception("Repository response code: ${response.code}"))
                val repo = response.parsedSafe<Repository>()
                logError(Exception("Parsed repository: ${repo?.name} with ${repo?.pluginLists?.size ?: 0} plugin lists"))
                repo
            } catch (e: Exception) {
                logError(Exception("Failed to parse repository: $url").apply { initCause(e) })
                null
            }
        }
    }
    ...
    private suspend fun parsePlugins(pluginUrls: String): List<SitePlugin> {
        // Take manifestVersion and such into account later
        return try {
            val fixedUrl = convertRawGitUrl(pluginUrls)
            logError(Exception("Parsing plugins from: $fixedUrl"))
            val response = app.get(fixedUrl, cacheTime = 5, cacheUnit = TimeUnit.MINUTES)
            logError(Exception("Plugins response code: ${response.code}"))
            val plugins = response.parsed<Array<SitePlugin>>().toList()
            logError(Exception("Parsed ${plugins.size} plugins"))
            plugins
        } catch (t: Throwable) {
            logError(Exception("Failed to parse plugins from: $pluginUrls").apply { initCause(t) })
            emptyList()
        }
    }
    ...
    suspend fun getRepoPlugins(repositoryData: RepositoryData): List<PluginWrapper>? {
        try {
            ...
            logError(Exception("Failed to get plugins for repo: ${repositoryData.name} (${repositoryData.url})").apply { initCause(e) })
            return null
        }
    }
    ...
    fun getRepositories(): Array<RepositoryData> {
        return getKey<Array<RepositoryData>>(REPOSITORIES_KEY, null) ?: emptyArray()
    }
    ...
    suspend fun removeRepository(context: Context, repository: RepositoryData) {
        ...
        repoLock.withLock {
            val currentRepos = getKey<Array<RepositoryData>>(REPOSITORIES_KEY, null) ?: emptyArray()
            // No duplicates
