...
import com.pulsestream.app.sync.FirestoreRestSyncManager
...
object DataStore {
    ...
    @Deprecated(
        "Please do not use the mapper version from DataStore. Preferably use methods from AppUtils " +
            ...
            "com.pulsestream.app.mapper to access the mapper directly if necessary.",
        level = DeprecationLevel.ERROR,
        replaceWith = ReplaceWith("com.pulsestream.app.mapper"),
    )
    ...
    fun Context.removeKey(path: String) {
        try {
            ...
            if (prefs.contains(path)) {
                ...
                // Fire-and-forget cloud delete; never blocks, never throws.
                FirestoreRestSyncManager.deleteKey(path)
            }
        } catch (e: Exception) {
            logError(e)
        }
    ...
    fun Context.removeKeys(folder: String): Int {
        ...
        try {
            ...
            // Fire-and-forget cloud deletes; never blocks, never throws.
            for (k in keys) {
                FirestoreRestSyncManager.deleteKey(k)
            }
            return keys.size
        } catch (e: Exception) {
            logError(e)
    ...
    fun <T> Context.setKey(path: String, value: T) {
        try {
            val json = value?.toJsonLiteral()
            getSharedPrefs().edit {
                putString(path, json)
            }
            // Fire-and-forget cloud write; never blocks, never throws.
            // Pass the serialized JSON string (not raw object) so Firestore
            // stores valid JSON, not Kotlin toString() output.
            FirestoreRestSyncManager.syncKey(path, json)
        } catch (e: Exception) {
            logError(e)
        }
    }
