...
import com.pulsestream.app.CloudStreamApp.Companion.getKeyClass
...
import com.pulsestream.app.CloudStreamApp.Companion.setKeyClass
...
import com.pulsestream.app.ui.player.ExtractorUri
import com.pulsestream.app.ui.player.NEXT_WATCH_EPISODE_PERCENTAGE
import com.pulsestream.app.ui.result.EpisodeSortType
import com.pulsestream.app.ui.result.ResultEpisode
import com.pulsestream.app.ui.result.VideoWatchState
...
object DataStoreHelper {
    ...
    fun setAccount(account: Account) {
        ...
        val oldKeyIndex = selectedKeyIndex
        ...
        // C2 FIX: Invalidate home page cache on account switch to prevent cross-account data leakage
        if (oldKeyIndex != account.keyIndex) {
            try {
                com.pulsestream.app.ui.home.HomePageCache.onAccountSwitch(account.keyIndex.toString())
            } catch (_: Exception) {}
        }

        // If switching to a different account, reset and re-initialize cloud sync
        // so bookmarks/history/settings from the new account are pulled from the cloud.
        if (oldKeyIndex != account.keyIndex) {
            try {
