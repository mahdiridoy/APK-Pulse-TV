...
object TestingUtils {
    ...
    class Logger {
        ...
        private val messageLog = Collections.synchronizedList(mutableListOf<Message>())

        fun getRawLog(): List<Message> = synchronized(messageLog) { messageLog.toList() }

        fun log(message: String) {
            synchronized(messageLog) { messageLog.add(Message(LogLevel.Normal, message)) }
        }

        fun warn(message: String) {
            synchronized(messageLog) { messageLog.add(Message(LogLevel.Warning, message)) }
        }

        fun error(message: String) {
            synchronized(messageLog) { messageLog.add(Message(LogLevel.Error, message)) }
        }
    }
...
//            return TestResult(validResults)
    ...
    private suspend fun testLinkLoading(
        ...
        val linksLoaded = java.util.concurrent.atomic.AtomicInteger(0)
        val invalidUrl = java.util.concurrent.atomic.AtomicReference<String?>(null)
        try {
            val success = api.loadLinks(url, false, {}) { link ->
                try {
                    logger.log("Video loaded: ${link.name}")
                    if (link.url.length <= 4 && invalidUrl.get() == null) {
                        invalidUrl.set("Api ${api.name} returns link with invalid url ${link.url}")
                    }
                    linksLoaded.incrementAndGet()
                } catch (t: Throwable) {
                    if (t is CancellationException) throw t
                }
            }
            if (success) {
                invalidUrl.get()?.let { fail(it) }
                logger.log("Links loaded: ${linksLoaded.get()}")
                return TestResult(linksLoaded.get() > 0)
            } else {
                fail("Api ${api.name} returns false on loadLinks() with ${linksLoaded.get()} links loaded")
            }
        } catch (e: Throwable) {
            when (e) {
                is NotImplementedError -> {
    ...
    fun getDeferredProviderTests(
        ...
        // Testing every provider at once floods the network and creates too many WebViews,
        // which crashes the app. Limit how many providers are tested at the same time.
